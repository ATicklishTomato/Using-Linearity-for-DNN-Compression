"""
This file contains an implementation of knowledge distillation with hints as described
by Romero et al. (2015) in "FitNets: Hints for Thin Deep Nets"
"""
import time
...
from compression_methods.basic_kd import ForwardKLLoss
...
class FeatureLoss(nn.Module):
    def __init__(self, input_dim, output_dim, device='cuda'):
        ...
        logger.info(f"Initializing FeatureLoss with input_dim={input_dim}, output_dim={output_dim}, device={device}")
...
def train_student_resnet(teacher_model, student_model, data_handler, optimizer,
    ...
    criterions = []
    if 0 in loss_layers:
        criterions.append(FeatureLoss(student_model.layer1[-1].conv2.out_channels, student_model.layer1[-1].conv2.out_channels, device))
    if 1 in loss_layers:
        criterions.append(FeatureLoss(student_model.layer2[-1].conv2.out_channels, student_model.layer2[-1].conv2.out_channels, device))
    if 2 in loss_layers:
        criterions.append(FeatureLoss(student_model.layer3[-1].conv2.out_channels, student_model.layer3[-1].conv2.out_channels, device))
    if 3 in loss_layers:
        criterions.append(FeatureLoss(student_model.layer4[-1].conv2.out_channels, student_model.layer4[-1].conv2.out_channels, device))
    final_criterion = ForwardKLLoss()
    ...
    for epoch in range(epochs):
        ...
        for inputs, labels in tqdm(train_loader, total=len(train_loader), desc="Training student ResNet model", leave=False,
            ...
            if 0 in loss_layers:
                teacher_model.layer1[-1].conv2.register_forward_hook(lambda x, y, z: hook(x, y, z, teacher_features))
                student_model.layer1[-1].conv2.register_forward_hook(lambda x, y, z: hook(x, y, z, student_features))
            if 1 in loss_layers:
                teacher_model.layer2[-1].conv2.register_forward_hook(lambda x, y, z: hook(x, y, z, teacher_features))
                student_model.layer2[-1].conv2.register_forward_hook(lambda x, y, z: hook(x, y, z, student_features))
            if 2 in loss_layers:
                teacher_model.layer3[-1].conv2.register_forward_hook(lambda x, y, z: hook(x, y, z, teacher_features))
                student_model.layer3[-1].conv2.register_forward_hook(lambda x, y, z: hook(x, y, z, student_features))
            if 3 in loss_layers:
                teacher_model.layer4[-1].conv2.register_forward_hook(lambda x, y, z: hook(x, y, z, teacher_features))
                student_model.layer4[-1].conv2.register_forward_hook(lambda x, y, z: hook(x, y, z, student_features))
            ...
            feature_loss = torch.stack([criterion(student_features[i], teacher_features[i]) for i, criterion in enumerate(criterions)], dim=0).mean()
            final_loss = final_criterion(teacher_outputs, student_outputs, labels)
            loss = final_loss + feature_loss
...
def distill_student_resnet(experimenter, data_handler, device='cuda', lr=2e-5,
                           epochs=5, blocks=None, loss_layers=None):
    """Distill a student ResNet model from a teacher ResNet model using feature-based knowledge distillation.
    Args:
        ...
        loss_layers (list): List of layer indices at which to compute loss
    Returns:
        student_model: ResNet model
        accuracy:           Top-1 accuracy of the model on the validation set.
        param_count:        Number of parameters in the model on the validation set.
    ...
    if loss_layers is None:
        loss_layers = [1, 3]
    elif max(loss_layers) >= 4:  # ResNet has 4 layers (0,1,2,3)
        raise ValueError(f"Loss layers should be between 0 and 3 for ResNet, got {loss_layers}")
    logger.info("Will compute loss at layers: " + ", ".join(str(i) for i in loss_layers))
    ...
    train_student_resnet(teacher_model, student_model, data_handler,
                         torch.optim.Adam(student_model.parameters(), lr=lr), loss_layers,
...
def train_student_llama(teacher_model, student_model, data_handler,
                        optimizer, loss_layers, device='cuda', epochs=5):
    ...
    Args:
        ...
        loss_layers: List of layer indices at which to compute loss
        device: torch.device
        epochs: number of epochs
    """
    teacher_model.to(device).eval()
    ...
    feature_criterions = [FeatureLoss(student_model.model.layers[i].self_attn.o_proj.out_features,
                                      teacher_model.model.layers[i].self_attn.o_proj.out_features).to(device) for i in loss_layers]
    final_criterion = ForwardKLLoss()
    train_loader = DataLoader(data_handler.train_set, batch_size=data_handler.batch_size, shuffle=True,
                                  num_workers=4,              # try 2–8 depending on CPU
                                  pin_memory=True,            # important for GPU transfer
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in enumerate(tqdm(train_loader, desc=f"Epoch {epoch + 1}/{epochs}",
            ...
            for i in loss_layers:
                teacher_model.model.layers[i].register_forward_hook(lambda x, y, z: hook(x, y, z, teacher_features))
                student_model.model.layers[i].register_forward_hook(lambda x, y, z: hook(x, y, z, student_features))

            with torch.autocast("cuda", dtype=torch.bfloat16):
                with torch.no_grad():
                    teacher_outputs = teacher_model(**inputs, labels=labels)
                student_outputs = student_model(**inputs, labels=labels)
                feature_loss = torch.stack([feature_criterions[i](teacher_outputs[0], student_outputs[0]) for i in range(len(loss_layers))], dim=0).mean()
                final_loss = final_criterion(student_outputs.logits, teacher_outputs.logits, labels)
                loss = final_loss + feature_loss
...
def distill_student_llama(experimenter, data_handler, device='cuda', lr=2e-5, epochs=5,
                          hidden_layer_reduction=2, top_k=5, loss_layers=None):
    """Evaluate a student Llama model from a teacher Llama model using feature-based knowledge distillation.
    Args:
        ...
        top_k: number of top k to check accuracy
        loss_layers (list): List of layer indices at which to compute feature loss.
    Returns:
    ...
    if loss_layers is None:
        max_layer_index =  len(student_model.model.layers) - 1
        loss_layers = [max_layer_index//2, max_layer_index]
    elif max(loss_layers) >= len(student_model.model.layers):
        raise ValueError(f"Loss layers should be between 0 and {len(student_model.model.layers)}")
    logger.info("Will compute loss at layers: " + ", ".join(str(i) for i in loss_layers))
...
def distill(experimenter, data_handler, device='cuda', lr=2e-5, epochs=5, top_k=5, blocks=None,
            hidden_layer_reduction=2, loss_layers=None):
    ...
    Args:
        ...
        loss_layers (list): List of layer or block indices after which to compute feature loss. If none, will default to [floor(0.5*|layers|), |layers|].
    Returns:
    ...
    if "resnet" in experimenter.model_name:
        return distill_student_resnet(experimenter, data_handler, device=device, lr=lr, epochs=epochs, blocks=blocks, loss_layers=loss_layers)
        ...
        return distill_student_llama(experimenter, data_handler, device=device, lr=lr, epochs=epochs,
                                     hidden_layer_reduction=hidden_layer_reduction, top_k=top_k, loss_layers=loss_layers)
    else:
