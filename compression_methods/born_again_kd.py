...
by Furlanello et al. (2018) in "Born Again Neural Networks"
"""
import time
...
def distill_student_resnet(experimenter, data_handler, device='cuda', lr=2e-5, epochs=5, block_layouts=None):
    """Distill a student ResNet model from a teacher ResNet model with an intermediate model inspired by Born-Again NNs.
    Args:
        ...
        lr: learning rate
        epochs: number of epochs
        block_layouts: The blocks list to be passed to ResNet constructor. Default is None, gets set to [2,2,2], aka one block of 2 less than ResNet-18
    Returns:
        student_model: ResNet model
        accuracy:           Top-1 accuracy of the model on the validation set.
        param_count:        Number of parameters in the model on the validation set.
        ...
        gflops:             GFLOPs during inference.
    """
    if block_layouts is None:
        blocks = [[1, 1, 2, 2], [1, 1, 1, 2]]
    else:
        blocks = block_layouts

    teacher_model = experimenter.model
    student_model = nn.Module() # Placeholder

    for block_layout in blocks:
        logger.info(f"Begin distilling student ResNet model with block layout: {block_layout}")
        student_model = get_student_resnet(blocks=block_layout, bottleneck=experimenter.model_name=="resnet50")
        ...
        logger.info("Finished training student ResNet model. Marking next teacher (unused if last pass).")
...
def distill_student_llama(experimenter, data_handler, device='cuda', lr=2e-5, epochs=5,
                          hidden_layer_reduction_iterations=None, top_k=5):
    """Evaluate a student Llama model from a teacher Llama model.
    Args:
        ...
        lr: learning rate
        epochs: number of epochs
        hidden_layer_reduction_iterations: Iterations of number of hidden layers to remove for student Llama model.
        top_k: number of top k to check accuracy
    Returns:
        ...
        gflops: GFLOPs of the model on the validation set.
    """
    if hidden_layer_reduction_iterations is None or len(hidden_layer_reduction_iterations) == 0:
        hidden_layer_reduction = [2, 3]
    else:
        hidden_layer_reduction = hidden_layer_reduction_iterations

    teacher_model = experimenter.model
    student_model = nn.Module() # Placeholder

    for hidden_layer_reduction in hidden_layer_reduction:
        logger.info(f"Hidden layer reduction: {hidden_layer_reduction}")
...
def distill(experimenter, data_handler, device='cuda', lr=2e-5, epochs=5, top_k=5, blocks_iterations=None,
            hidden_layer_reduction_iterations=None):
    ...
    Args:
        ...
        blocks_iterations: Blocks layouts for the student ResNet models. Default is [[1,1,2,2],[1,1,1,2]]. Ignored for Llama model.
        hidden_layer_reduction_iterations (list): Iterations of number of hidden layers to remove for student Llama model. Default is 2, meaning that 18 layers of Llama-3.2-1b will become 16 layers. Ignored for ResNet model.
    Returns:
    ...
    if "resnet" in experimenter.model_name:
        return distill_student_resnet(experimenter, data_handler, device=device, lr=lr, epochs=epochs, block_layouts=blocks_iterations)
        ...
        return distill_student_llama(experimenter, data_handler, device=device, lr=lr, epochs=epochs,
                                     hidden_layer_reduction_iterations=hidden_layer_reduction_iterations, top_k=top_k)
    else:
