...
def benchmark_compression_methods(model_name, dataset, batch_size, epochs, lr, max_batches, save, seed, device,
    ...
    data_handler = DataManager(dataset_name=dataset, batch_size=batch_size, model_name=model_name,
    ...
    if "llama" in model_name:
        experimenter = LlamaExperimenter(model_name=model_name, data_handler=data_handler, batch_size=batch_size, epochs=epochs,
                                         learning_rate=lr, max_batches=max_batches, device=device, save=save)
    ...
    if save:
        torch.save(student_model.state_dict(), f"./results/{model_name}_distilled.pth")
        logger.info("Saved models to ./results directory.")
        ...
        with open(f"./results/{model_name}_compression_benchmark_results.json", "w") as f:
