...
def mean_preactivations(model, data_handler, device='cuda', save=False, save_dir="./results"):
    ...
    with torch.no_grad():
        if "llama" in save_path.lower():
            data_loader = DataLoader(dataset, batch_size=data_handler.batch_size, shuffle=True,
            ...
            for batch in tqdm(data_loader, total=len(data_loader), desc="Processing samples for preactivations", leave=False, disable=debug_mode):
